...
export async function GET(request: Request) {
  ...
  const result = await patentsService.getPatentEvidence(metadata.company);
  await evidenceRepository.saveEvidence(result.items);
  await evidenceRepository.saveProviderStatuses(result.statuses);
  return NextResponse.json(createApiResponse(result, "GET /api/evidence/patents?ticker=MSFT"));
}
