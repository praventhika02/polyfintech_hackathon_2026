...
export async function GET(request: Request) {
  ...
  const result = await filingsService.getFilingEvidence(metadata.company);
  await evidenceRepository.saveEvidence(result.items);
  await evidenceRepository.saveProviderStatuses(result.statuses);
  return NextResponse.json(createApiResponse(result, "GET /api/evidence/filings?ticker=D05.SI"));
}
